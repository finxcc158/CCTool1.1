            ...
            try
            {
                ...
                await QueuedTask.Run(() =>
                {
                    ...
                    foreach (var village_name in village_names)
                    {
                        ...
                        pw.AddMessage("1、生成村域综合现状图", Brushes.Gray);
                        // 1、生成村域综合现状图
                        VG.CreateXZT(village_name, pw, time_base);

                        pw.AddProcessMessage(10, time_base, "2、生成村域综合规划图", Brushes.Gray);
                        // 2、生成村域综合规划图
                        VG.CreateGHT(village_name, pw, time_base);

                        pw.AddProcessMessage(10, time_base, "3、生成国土空间管制边界图", Brushes.Gray);
                        // 3、生成国土空间管制边界图
                        VG.CreateGKBJT(village_name, pw, time_base);
                    }
                });
